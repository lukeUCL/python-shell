...
class TestCut(unittest.TestCase):
    def test_one(self):
        ...
        self.assertEqual(Cut().exec(['-b', '1', 'test/text.txt']).rstrip().split('\n'), ["h", "c", "i"])
    ...
    #test range where second element begins with -3 that cut functions correctly
    def test_range4(self):
        self.assertEqual(Cut().exec(['-b', '-3', 'test/text.txt']).rstrip().split('\n'), ["hel", "cor", "i t"])
...
class testSort(unittest.TestCase):
    ...
    def test_sort_file(self):
        self.assertEqual(Sort().exec(['test/text.txt']).rstrip().split('\n'), ["corgito ergo sum", "hello world", "i think therefore i am"])
...
class testFind(unittest.TestCase):
    ...
    def test_find(self):
        self.assertEqual(Find().exec(['test', '-name', 'text.txt']).rstrip().split('\n'), ["test/text.txt"])
    #test that find finds the correct file with -name flag
    def test_find_name(self):
        self.assertEqual(Find().exec(['-name', 'text.txt']).rstrip().split('\n'), ["./test/text.txt"])
